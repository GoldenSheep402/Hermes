...
import type { InnetTracker, Settings } from '@/lib/proto/system/v1/system.pb.ts'
      ...
      <a-form :model="setting" @submit="handleSubmit">
        ...
        <a-form-item field="trackers" label="内网跟踪器">
          <div class="flex flex-col">
            <a-button type="primary" @click="trackers.push({ addr: '', enable: false })">
              添加
            </a-button>
            <div v-for="(tracker, index) in trackers" :key="index">
              <div class="m-t2 flex flex-row items-center gap-2">
                <a-input v-model="trackers[index].addr" />
                <a-button :type="setButtonStatus(trackers[index].toBeDeleted)" @click="deleteTracker(index)">
                  删除
                </a-button>
                <a-switch v-model="trackers[index].enable" class="flex justify-center" />
              </div>
            </div>
          </div>
        </a-form-item>
